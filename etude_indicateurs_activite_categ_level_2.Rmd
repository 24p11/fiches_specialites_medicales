...
dplyr::full_join(df_spe_categ,df_spe_tot_region) |> 
  ...
  dplyr::filter(pm>10) |> 
  distinct(categ) |> dplyr::pull(categ) -> groups
...
if(specialite == "A-Total"){
  ...
  criteres = list(age2 =age_ref,lib_spe_uma = specialite,niveau = "Total Spécialité régional",annee = c(2018,2024),type_hosp=NA)
...
df_hist<-get_df_hist_categ(df |> dplyr::filter(annee %in% c(2018,2024),is.na(type_hosp)),age_ref,indicateur_ref,specialite,groups)
...
df |> filter_df(list(age2 = age_ref,niveau=c("Spécialité - GHU"), categ="APHP")) |> 
  ...
  #Ajout du total GHU
  dplyr::left_join(df |> filter_df(list(age2 = age_ref,niveau=c("Total categ"),  categ="APHP")) |> 
