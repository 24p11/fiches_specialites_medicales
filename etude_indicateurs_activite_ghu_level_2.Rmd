...
df_spe_categ<-df |> filter_df(list(age2 = age_ref,
                                   type_hosp=NA,niveau = "Spécialité - Categ",
                                   lib_spe_uma= specialite, niveau =c("Spécialité - Categ"))) |>
  distinct(annee,categ,tot) |> 
  dplyr::rename(nb = tot)

df_spe_tot_region<- df |> filter_df(list(age2 = age_ref,
                                         type_hosp=NA,niveau = "Total Spécialité régional",
                                         lib_spe_uma= specialite, niveau =c("Spécialité - Categ")))  |> 
  distinct(annee,tot)
...
df |> dplyr::mutate(categ = ifelse(! is.na(ghu),ghu,categ)) |> 
  ...
  dplyr::distinct(annee,categ,ghu,tot) |>
  dplyr::rename(nb = tot) |> 
  dplyr::mutate(categ = ifelse(categ==ghu_ref,"APHP " %+% ghu_ref,categ)) ->df_BARPLOT

# Calcul des effectifs APHP sans GHU REF 
df_BARPLOT |> dplyr::select(annee,categ,nb) |> 
  dplyr::filter(categ =="APHP " %+% ghu_ref) |>
  dplyr::mutate(categ = "APHP") |> 
  dplyr::rename(nb_diff = nb) -> df_diff

# Ajout des effectifs APHP hors GHU REF au dataframe df_BARPLOT
df_BARPLOT <- df_BARPLOT |> dplyr::left_join(df_diff) |> 
  dplyr::mutate(nb_diff = ifelse(is.na(nb_diff),0,nb_diff),nb= nb-nb_diff) |> 
  dplyr::select(-nb_diff)
...
```{r download-link-synthese, echo=FALSE, results='asis'}
# Générer le contenu CSV
csv_content <- capture.output(
  write.table(df_html, file = stdout(), row.names = FALSE, na = "", fileEncoding =  "CP1252",sep = ";")
)
csv_content <- paste(csv_content, collapse = "\r\n")

# 2. Ajouter le BOM (Byte Order Mark) pour Excel
bom <- charToRaw("\xEF\xBB\xBF")  # BOM pour UTF-8 (Excel le reconnaît mieux)
csv_with_bom <- rawToChar(c(bom, charToRaw(csv_content)))

# 3. Encoder en base64 pour éviter les problèmes
csv_base64 <- base64enc::base64encode(charToRaw(csv_with_bom))

# Générer le bouton avec JavaScript
cat(sprintf('
<button onclick="downloadCSV(\'%s\')" style="padding: 8px 16px; background-color: #007bff; color: white; border: none; border-radius: 4px; cursor: pointer;">
  Télécharger en CSV
</button>
...
function downloadCSV(base64Data) {
  ...
  link.download = "synthse_periode.csv";
...
if(specialite =="A-Total"){
  ...
  criteres <- list(categ=groups,age2 = age_ref,lib_spe_uma = specialite,niveau = c("Spécialité - Categ","Spécialité - GHU"))
...
```{r download-link-evolution, echo=FALSE, results='asis'}
# Générer le contenu CSV
csv_content <- capture.output(
  write.table(df_html, file = stdout(), row.names = FALSE, na = "", fileEncoding =  "CP1252",sep = ";")
)
...
function downloadCSV(base64Data) {
  ...
  const link = document.createElement("a");
  link.href = URL.createObjectURL(blob);
  link.download = "evolution_hc_hp.csv";
  document.body.appendChild(link);
  link.click();
  document.body.removeChild(link);
}
...
df_hist<-get_df_hist_ghu(df |> dplyr::filter(annee %in% periode_annee,is.na(type_hosp)),ghu_ref,age_ref,indicateur_ref,specialite,groups)
...
```{r, echo=FALSE, results='asis'}
if(age_ref=="ge_18" & specialite!="OBSTETRIQUE"){
  cat("#### Patients gériatriques")
  indicateur_ref = "nb_geriatrie"

  df_hist<-get_df_hist_ghu(df|> dplyr::filter(annee %in% c(min(periode_annee),max(periode_annee)),is.na(type_hosp)),
                           ghu_ref,age_ref,indicateur_ref,specialite,groups)
...
df |> filter_df(list(age2 = age_ref,niveau=c("Spécialité - GHU"), ghu=ghu_ref)) |> 
  dplyr::distinct(age2,lib_spe_uma,type_hosp,tot,annee) |> 
  dplyr::mutate(type_hosp = ifelse(is.na(type_hosp),"tot",type_hosp)) |> 
  dplyr::rename(nb= tot) |> 
  #Ajout du total APHP par spécialité
  dplyr::left_join( df |> filter_df(list(age2 = age_ref,niveau=c("Spécialité - Categ"), categ="APHP")) |> 
                    dplyr::distinct(age2,lib_spe_uma,type_hosp,tot,annee) |> 
                    dplyr::mutate(type_hosp = ifelse(is.na(type_hosp),"tot",type_hosp)) |> 
...
```{r download-link-carto, echo=FALSE, results='asis'}

if(specialite=="A-Total"){
  # Générer le contenu CSV
  csv_content <- capture.output(
    write.table(df_res, file = stdout(), row.names = FALSE, na = "", fileEncoding =  "CP1252",sep = ";")
  )
  csv_content <- paste(csv_content, collapse = "\r\n")
